...
<body class="bg-light">
    <div class="parent">
        ...
        <%-- Đã sửa đường dẫn Header.jsp thành tuyệt đối --%>
        ...
        <div class="div3 px-4 pt-4">
            ...
            <form action="${pageContext.request.contextPath}/ComplaintServlet" method="get" class="mb-4">
                <div class="row g-3">
                    <div class="col-md-4">
                        <input type="text" name="search" class="form-control" placeholder="Tìm kiếm theo tên người dùng hoặc mô tả" value="${searchTerm != null ? searchTerm : ''}">
                    </div>
                    <div class="col-md-3">
                            ...
                            <c:when test="${not empty complaints}">
                                <c:forEach var="complaint" items="${complaints}">
                                    <tr>
                                        ...
                                        <td>
                                            <a href="${pageContext.request.contextPath}/ComplaintServlet?action=view&issueId=${complaint.issueId}" class="btn btn-sm btn-info">Chi tiết / Trả lời</a>
                                        </td>
                                    </tr>
                                </c:forEach>
                            </c:when>
        ...
        <%-- Đặt phần phân trang ở đây, ngay sau div3 --%>
        <div style="text-align: center; padding: 20px 0; background-color: #f8f9fa; grid-column: 1 / -1;">
            <nav aria-label="Page navigation">
                <ul class="pagination justify-content-center">
                    <li class="page-item ${currentPage == 1 ? 'disabled' : ''}">
                        <a class="page-link" href="${pageContext.request.contextPath}/ComplaintServlet?page=${currentPage - 1}&search=${searchTerm != null ? searchTerm : ''}&statusFilter=${statusFilter != null ? statusFilter : ''}&priorityFilter=${priorityFilter != null ? priorityFilter : ''}" aria-label="Previous">
                            <span aria-hidden="true">&laquo;</span>
                        </a>
                    </li>
                    <c:forEach var="i" begin="1" end="${totalPages}">
                        <li class="page-item ${currentPage == i ? 'active' : ''}">
                            <a class="page-link" href="${pageContext.request.contextPath}/ComplaintServlet?page=${i}&search=${searchTerm != null ? searchTerm : ''}&statusFilter=${statusFilter != null ? statusFilter : ''}&priorityFilter=${priorityFilter != null ? priorityFilter : ''}">${i}</a>
                        </li>
                    </c:forEach>
                    <li class="page-item ${currentPage == totalPages ? 'disabled' : ''}">
                        <a class="page-link" href="${pageContext.request.contextPath}/ComplaintServlet?page=${currentPage + 1}&search=${searchTerm != null ? searchTerm : ''}&statusFilter=${statusFilter != null ? statusFilter : ''}&priorityFilter=${priorityFilter != null ? priorityFilter : ''}" aria-label="Next">
